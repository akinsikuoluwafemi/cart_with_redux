  ...
  switch (action.type) {
    ...
    case FETCH_PRODUCTS:
      ...
      console.log(state)
        ...
        // let tempCartArr = state.cartItems.map((item) => parseInt(item.count) + 1);
        // console.log( tempCartArr)
      let tempCartArr = state.products.find(item => item.id === action.payload)
      console.log(tempCartArr)
      tempCartArr.count = tempCartArr.count + 1;
      tempCartArr.total = tempCartArr.count * product.price;


        return { ...state, cartItems: tempCartArr  };
