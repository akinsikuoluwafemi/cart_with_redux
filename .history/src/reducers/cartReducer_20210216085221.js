...
const cartReducer = (state=initialState, action) => {
    switch (action.type) {
      ...
      case ADD_TOTALS:
        let subTotal = 0;
        // let cartSt = state.products.map((item) => (subTotal += item.total));
        const tempTax = subTotal * 0.1;
        const tax = parseFloat(tempTax.toFixed(2));
        const total = subTotal + tax;
        return { ...state, cartSubTotal: state.products.map((item) => (subTotal += item.total)), cartTotal: total, cartTax: tax  }
